...
export default function SyllabusSection() {
  // Get URL search parameters to check for semester selection
  ...
  useEffect(() => {
    ...
    if (sem && sem >= 1 && sem <= 6) {
      setActiveTab(`sem${sem}`)
    }
  }, [searchParams])

  // Listen for custom semester change events from header navigation
  useEffect(() => {
    const handleSemesterChange = (event) => {
      const sem = event.detail.semester
      if (sem && sem >= 1 && sem <= 6) {
        setActiveTab(`sem${sem}`)
      }
    }

    // Add event listener for custom semesterChange event
    window.addEventListener("semesterChange", handleSemesterChange)

    // Clean up event listener on component unmount
    return () => {
      window.removeEventListener("semesterChange", handleSemesterChange)
    }
  }, [])
